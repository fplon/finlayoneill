      ...
      <div className="space-y-5 print:space-y-3">
        {EXPERIENCE.map((exp: ExperienceType, index: number) => (
          <div key={index} className="break-inside-avoid">
            ...
            <p className="text-md font-medium text-gray-600 mb-1 print:text-sm print:font-normal print:mb-0.5">
              {exp.company}
            </p>
            <p className="text-gray-700 text-sm mb-2 print:text-xs print:leading-snug">
              {exp.description}
            </p>
            {exp.technologies && exp.technologies.length > 0 && (
              <div className="flex flex-wrap gap-1 mt-1 print:gap-0.5">
                {exp.technologies.map((tech: string, techIndex: number) => (
                  <span
                    key={techIndex}
                    className="bg-gray-200 text-gray-700 text-xs font-medium px-2 py-0.5 rounded-full print:bg-gray-100 print:text-gray-600 print:px-1 print:py-0 print:text-[10px] print:rounded"
                  >
                    {tech}
                  </span>
                ))}
